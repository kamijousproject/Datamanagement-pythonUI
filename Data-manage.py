...
class PhoneDataManager:
        ...
        try:
            ...
            for idx, dataset in enumerate(selected_datasets):
                cursor.execute(f"""
                    ...
                    FROM {source_table} WHERE dataset_name = %s
                """, (dataset,))
                ...
                if records:
                    ...
                    if delete_after_move:
                        cursor.execute(
                            f"DELETE FROM {source_table} WHERE dataset_name = %s", (dataset,))
                        conn.commit()
    ...
    def export_selected_data(self):
        ...
        for row in selected:
            ...
            try:
                ...
                query = f"""
                    SELECT id, phone_number FROM {table}
                    WHERE dataset_name = %s AND data_type = %s
                """
    ...
    def show_duplicates_preview(self):
        ...
        try:
            ...
            for phone in self.phone_numbers:
                cursor.execute(
                    f"SELECT COUNT(*) FROM {table} WHERE phone_number = %s", (phone,))
    ...
    def save_to_database(self):
        ...
        try:
            ...
            progress_window.geometry("400x100")
            progress_window.resizable(False, False)
            tk.Label(progress_window, text="กำลังนำเข้าข้อมูล กรุณารอสักครู่...", font=(
                "Kanit", 10)).pack(pady=(10, 5))
            ...
            progress_status = tk.Label(
                progress_window, text="", font=("Kanit", 9))
            progress_status.pack()
            ...
            for phone in self.phone_numbers:
                if phone in existing_numbers:
                    ...
                    new_entries.append((
                        phone, dataset_name, receive_date, source, detail, data_type
                    ))
            ...
            for i in range(0, total, batch_size):
                ...
                progress_var.set(percent)
                progress_status.config(
                    text=f"นำเข้าแล้ว {i + len(batch)} / {total} เบอร์")
                progress_window.update()
